...
Validating that this was actually a bug and not an incorrect query actually took the most time of the whole validation task, because it changed the result in very subtle ways and always disappeared when I removed any essential aspect in order to produce a minimal working example for a bug report.
When I finally confirmed it, I created a workaround of several steps using the Linux `comm` tool:

 1. remove `filter not exists {?s rdfs:subClassOf+ ?domain.}`, save result as TSV in file `all`
